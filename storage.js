...
function saveToStorage(state) {
  ...
  localStorage.setItem(STORAGE_KEYS.smartPauseRate, state.smartPauseRate || 0.3);
  ...
  if (state.title !== undefined) {
    localStorage.setItem(STORAGE_KEYS.title, state.title || '');
  }
}
...
function loadFromStorage() {
  ...
  try {
    ...
    const spr = localStorage.getItem(STORAGE_KEYS.smartPauseRate);
    if (spr) state.smartPauseRate = parseFloat(spr);
...
function importText(content) {
  ...
  return lines.map(line => createRowData(line.trim(), 2));
}
