...
function renderRows(app, $rowsContainer, dragHandler, rowPlayHandler, deleteHandler, saveFn) {
  ...
  app.rows.forEach((row, idx) => {
    ...
    $pause.dataset.pause = row.pauseSeconds;
    const $pauseLabel = document.createElement('span');
    $pauseLabel.className = 'row-pause-label';
    $pauseLabel.textContent = '停顿';
    const $pauseInput = document.createElement('input');
    $pauseInput.type = 'number';
    $pauseInput.className = 'row-pause-input';
    $pauseInput.value = row.pauseSeconds;
    $pauseInput.setAttribute('value', row.pauseSeconds);
    $pauseInput.min = 1;
    $pauseInput.step = 1;
    $pauseInput.disabled = app.globalPause || app.smartPause;
    $pauseInput.addEventListener('input', () => {
      let v = parseInt($pauseInput.value);
      if (isNaN(v) || v < 1) v = 1;
      $pauseInput.value = v;
      $pauseInput.setAttribute('value', v);
      row.pauseSeconds = v;
      $pause.dataset.pause = v;
      if ($pausePrint) $pausePrint.textContent = v;
      if (saveFn) saveFn();
    });
    const $pauseUnit = document.createElement('span');
    $pauseUnit.className = 'row-pause-unit';
    $pauseUnit.textContent = '秒';
    const $pausePrint = document.createElement('span');
    $pausePrint.className = 'row-pause-print';
    $pausePrint.textContent = row.pauseSeconds;
    $pause.appendChild($pauseLabel);
    $pause.appendChild($pauseInput);
    $pause.appendChild($pausePrint);
    $pause.appendChild($pauseUnit);
...
function updateButtonStates(app) {
  ...
  if ($btnPause) $btnPause.disabled = !hasRows && app.state === 'idle';
  if ($btnStop) $btnStop.disabled = !isPlaying;
  if ($btnPrev) $btnPrev.disabled = !isPlaying;
  if ($btnNext) $btnNext.disabled = !isPlaying;

  // Update pause button: speaking=暂停, others=播放
  if ($btnPause) {
    if (app.state === 'speaking') {
      $btnPause.innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><rect x="4" y="3" width="3" height="10" rx="0.5"/><rect x="9" y="3" width="3" height="10" rx="0.5"/></svg> 暂停';
      $btnPause.title = '暂停';
      $btnPause.classList.add('primary');
    } else {
      $btnPause.innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M5 3.5l8 4.5-8 4.5V3.5z"/></svg> 播放';
      $btnPause.title = '播放';
      $btnPause.classList.add('primary');
    }
  }
  ...
  app.rows.forEach(row => {
    ...
    if (row._pauseInputEl) row._pauseInputEl.disabled = isPlaying || app.globalPause || app.smartPause;
  });
}
