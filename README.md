...
一个简洁的引导式跟读提词器，适合演讲、朗诵、外语跟读、无纸化拍摄、主持、口播、视障辅助阅读等场景。
...
- **自定义标题**：点击顶部标题即可修改，自动保存到本地。
- **打印排版**：一键打印，只保留标题、文稿和每行停顿秒数，便于纸质排练。
- **夜间模式**：一键切换深色主题，保护眼睛。
...
1. 用浏览器打开 `index.html`。
2. 点击顶部标题可修改本次练习的标题（自动保存）。
3. 在输入框中填入要练习的文稿，每行一句话。
4. 点击"播放"，系统会领读一句，然后给出停顿时间供你朗读。
5. 使用"上一句"/"下一句"跳转，或按空格键暂停/继续。
6. 点击"打印"可生成适合纸质排练的排版。
...
语音合成功能依赖浏览器对 Web Speech API 的支持，部分国产浏览器可能不支持，请使用 Chrome、Edge、Safari 等现代浏览器。
